...
def generate_copilot_response(user_prompt: str, user_email: str, company_name: str, res: dict) -> dict:
  ...
  if any(w in q for w in ["help me navigate", "how to navigate", "menu", "navigation guide", "where can i go", "where to go", "how to use"]):
    text = (
      ...
      f"- **Circular Economy (4R)**: Explore industrial scrap reuse, recycling, and material alternatives.\n"
      f"- **Business Profile & Setup**: Manage facility parameters and baseline quotas.\n\n"
  ...
  if any(w in q for w in ["personalize", "personal info", "my info", "company info", "my company", "my details", "profile info", "about me", "who am i", "overview of my business"]):
    ...
    text = (
      ...
      f"- **Net Compliance Position**: `{'Deficit: ' + f'{credits_required:,.1f} tonnes' if is_deficit else 'Surplus: ' + f'{credits_remaining:,.1f} credits'}`\n"
  ...
  if any(w in q for w in ["daily credit", "weekly credit", "expected credit", "expected carbon", "government issue", "govt issue", "initially issue", "initial quota", "initial credit", "cross check", "crosscheck", "statutory quota", "daily use", "weekly use"]):
    ...
    status_badge_line = f"Projected Deficit of {audit['projected_credits_needed']:,.1f} credits" if audit['is_projected_deficit'] else f"Projected Surplus of {audit['projected_surplus_credits']:,.1f} credits"
    
    text = (
      f"### Carbon Credit Quota Audit & Burn Rate for {company_name}\n\n"
      f"- **Government Initial Quota Issued**: `{audit['initial_govt_quota']:,.0f} credits` (Calibrated under `{audit['regulatory_regime']}` for {audit['employees']} employees)\n"
  ...
  if any(w in q for w in ["credit", "deficit", "quota", "compliance cost", "penalty", "market", "liability", "balance"]):
    if is_deficit:
      text = (
        f"### Carbon Credit Deficit Warning for {company_name}\n\n"
  ...
  if any(w in q for w in ["leak", "hotspot", "loss", "biggest source", "diagnostics", "where are"]):
    ...
    return {"text": text, "nav_target": "leak_detection", "suggestions": ["Open Leak Diagnostics", "Go to Dashboard", "Circular Alternatives"]}
  ...
  if any(w in q for w in ["score", "rating", "benchmark", "grade", "eco score"]):
    text = (
      ...
      f"- **Status**: {'Above statutory benchmark 🌟' if s_score >= 70 else 'Requires targeted Scope 1/2 efficiency measures '}\n\n"
...
def render_copilot_chat(key_prefix: str = "copilot"):
  ...
  st.markdown(f"""
    <div style="background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 12px; padding: 12px 16px; margin-bottom: 12px; box-shadow: var(--shadow-card);">
      <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
        <div>
          <div style="font-size: 0.72rem; font-weight: 800; text-transform: uppercase; color: #5A8766; letter-spacing: 0.08em;">
        ...
        <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
          ...
          <div style="background: var(--bg-subtle); padding: 5px 10px; border-radius: 8px; border: 1px solid var(--border-color); text-align: center;">
            ...
            <div style="font-weight: 800; font-size: 0.85rem; color: {'#EF4444' if is_deficit else '#5A8766'};">
          ...
          <div style="background: var(--bg-subtle); padding: 5px 10px; border-radius: 8px; border: 1px solid var(--border-color); text-align: center;">
            ...
            <div style="font-weight: 800; font-size: 0.92rem; color: #5A8766;">{s_score:.0f}/100</div>
          </div>
          <div style="background: var(--bg-subtle); padding: 5px 10px; border-radius: 8px; border: 1px solid var(--border-color); text-align: center;">
  ...
  with n_c4:
    if st.button("Carbon Credits", key=f"{key_prefix}_nav_credits", use_container_width=True):
      ...
      st.rerun()
  with n_c5:
    if st.button("Circular 4R", key=f"{key_prefix}_nav_circ", use_container_width=True):
      st.session_state["nav_section"] = "circular"
      st.session_state["current_step"] = 10
      st.rerun()
  with n_c6:
    if st.button("Setup", key=f"{key_prefix}_nav_setup", use_container_width=True):
      ...
      if msg["sender"] == "user":
        ...
        st.markdown(f"""
          <div class="chat-bubble-bot">
            <div style="display: flex; align-items: center; margin-bottom: 4px; font-weight: 700; color: #5A8766; font-size: 0.8rem;">
...
def render_copilot_view():
  ...
  header_icon = feather_icon("cpu", color="#5A8766", size=30, margin_right=10)
  st.markdown(f"""
