...
def render_data_entry_view():
    ...
    if is_quick_mode:
      ...
      with c_q2:
        ...
        q_waste = st.number_input(
          "Total Solid Waste Generated (kg / yr)",
          min_value=0.0,
          max_value=10000000.0,
          value=def_waste,
          step=500.0,
  ...
  if submit_btn:
    if is_quick_mode:
      ...
      if tot_act <= 0:
        st.error("Please enter at least one operational activity volume above zero.")
        return

      if q_elec == 0 and ("Retail" in btype or "Food" in btype):
        st.warning("Warning: Electricity is set to 0. Commercial facilities typically have refrigeration/lighting baseline.")

      if q_renew < 0 or q_renew > 100:
        st.error("Renewable energy share must be between 0% and 100%.")
        return
