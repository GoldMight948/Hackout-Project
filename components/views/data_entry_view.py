...
try:
    from components.calculations import calculate_detailed_emissions, safe_float, calculate_emissions, INDUSTRY_BENCHMARKS, get_statutory_carbon_quota
except ImportError:
    from components.calculations import calculate_detailed_emissions, calculate_emissions, INDUSTRY_BENCHMARKS, get_statutory_carbon_quota
...
def render_data_entry_view():
    ...
    with st.form("operational_data_entry_form"):
        ...
        if is_quick_mode:
            ...
            def_price = safe_float(inputs.get("credit_price") if inputs.get("credit_price") is not None else inputs.get("carbon_credit_price"), quota_calc["benchmark_price"])
