...
def render_activity_log_view():
    ...
    with tab_entry:
        ...
        if just_logged:
            ...
            with col_act1:
                if st.button(f"➕ Log Next Day ({next_day_val.strftime('%b %d, %Y')})", key="act_log_next_day_btn", type="primary", use_container_width=True):
                    ...
                    st.rerun()
            with col_act2:
                if st.button(f"🗑️ Delete Entry #{just_logged['id']}", key=f"act_del_just_{just_logged['id']}", type="secondary", use_container_width=True):
        ...
        with st.form("activity_entry_form"):
            ...
            with c_w5:
                in_haz = st.number_input("☣️ Hazardous Waste (kg)", min_value=0.0, value=0.0, step=5.0, key="act_haz_inp")
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            if submit_log:
                ...
                if total_activity <= 0:
                    st.error("⚠️ Please enter at least one positive operational quantity (fuel, waste, electricity, or transport).")
                else:
                    log_payload = {
        ...
        if not logs:
            ...
            for l in logs[:8]:
                ...
                with c_stat:
                    ...
                    """, unsafe_allow_html=True)
                with c_del:
                    if st.button("🗑️ Delete", key=f"inline_del_log_{l['id']}", type="secondary", use_container_width=True):
