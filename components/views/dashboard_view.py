...
def render_dashboard_view():
    ...
    with tab_annual:
        ...
        if agg_stat["total_entries"] > 0:
            sync_icon = feather_icon("refresh-cw", color="#10B981", size=16, margin_right=8)
            st.markdown(f"""
        ...
        # =========================================================================
        audit_icon = feather_icon("award", color="#10B981", size=20, margin_right=8)
        st.markdown(f"""
        ...
        with ac4:
            exp_color = "#EF4444" if audit_data['is_projected_deficit'] else "#10B981"
        ...
        status_badge_daily = '<span class="badge-low" style="padding: 3px 8px; font-size: 0.75rem;">🟢 WITHIN DAILY CAP</span>' if audit_data['actual_daily_avg'] <= audit_data['daily_quota_target'] else '<span class="badge-critical" style="padding: 3px 8px; font-size: 0.75rem;">⚠️ EXCEEDS DAILY CAP</span>'
        status_badge_weekly = '<span class="badge-low" style="padding: 3px 8px; font-size: 0.75rem;">🟢 WITHIN WEEKLY CAP</span>' if audit_data['actual_weekly_avg'] <= audit_data['weekly_quota_target'] else '<span class="badge-medium" style="padding: 3px 8px; font-size: 0.75rem;">⚠️ FREIGHT / LOGISTICS SURGE</span>'
                ...
                <table class="saas-table" style="width: 100%; font-size: 0.88rem; border-collapse: collapse;">
                    <thead>
                        <tr style="border-bottom: 2px solid var(--border-color); text-align: left;">
                            ...
                            <th style="padding: 10px 12px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; font-size: 0.75rem;">🏛️ Govt Issued Quota Allowance</th>
                    ...
                    <tbody>
                        <tr style="border-bottom: 1px solid var(--border-color);">
                            <td style="padding: 10px 12px; font-weight: 600;">🌅 <strong>Daily Carbon Credits</strong></td>
                        ...
                        <tr style="border-bottom: 1px solid var(--border-color);">
                            <td style="padding: 10px 12px; font-weight: 600;">🗓️ <strong>Monthly Run-Rate</strong></td>
                        ...
                        <tr style="border-bottom: 1px solid var(--border-color); background: rgba(16, 185, 129, 0.03);">
                            <td style="padding: 10px 12px; font-weight: 700; color: #065F46;">🏛️ <strong>Annual Statutory Quota</strong></td>
                            <td style="padding: 10px 12px; font-weight: 700; color: #059669;"><strong>{audit_data['initial_govt_quota']:,.0f}</strong> credits initially issued</td>
                            ...
                            <td style="padding: 10px 12px; font-weight: 700; color: {'#DC2626' if audit_data['is_projected_deficit'] else '#059669'};"><strong>{audit_data['expected_annual_burn']:,.1f}</strong> credits expected full year</td>
                            <td style="padding: 10px 12px;">{status_badge_annual}</td>
                        </tr>
                        <tr>
                            <td style="padding: 10px 12px; font-weight: 600;">🏦 <strong>Remaining Quota in Bank</strong></td>
    ...
    with tab_periodic:
        ...
        if dash_just_logged:
            ...
            st.markdown(f"""
                <div style="background: rgba(16, 185, 129, 0.12); border: 1px solid #6EE7B7; border-radius: 8px; padding: 10px 14px; margin-bottom: 14px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px;">
                    <div style="display: flex; align-items: center;">
                        {feather_icon('check-circle', color='#059669', size=18, margin_right=8)}
            ...
            with cq_a1:
                if st.button(f"➕ Log Next Day ({next_q_date.strftime('%b %d')})", key="dash_log_next_day_btn", type="primary", use_container_width=True):
                    ...
                    st.rerun()
            with cq_a2:
                if st.button(f"🗑️ Delete Entry #{dash_just_logged['id']}", key="dash_del_just_logged_btn", type="secondary", use_container_width=True):
        ...
        with st.expander("⚡ Quick Log Operational Activity (Fuel, Waste, Utilities)", expanded=False):
            with st.form("dash_quick_log_form"):
                ...
                with cq3:
                    ql_waste = st.number_input("🗑️ Solid Waste (kg)", min_value=0.0, value=0.0, step=20.0, key="ql_waste")
        ...
        if not p_logs or p_agg["total_entries"] == 0:
            ...
            with pk4:
                p_exp_color = "#EF4444" if audit_data['is_projected_deficit'] else "#10B981"
                st.markdown(f"""
                    <div class="kpi-card {'deficit' if audit_data['is_projected_deficit'] else 'low'}">
                        <div class="kpi-title">
            ...
            with col_waste:
                ...
                if w_values:
                    fig_w_donut = go.Figure(data=[go.Pie(
                        ...
                        values=w_values,
                        hole=0.6,
                        marker=dict(colors=["#10B981", "#EF4444", "#6B7280", "#F59E0B", "#991B1B"]),
            ...
            with col_table:
                ...
                for l in p_logs[:6]:
                    ...
                    with c_ts:
                        ...
                        cap_color = "#10B981" if pct_cap <= 100 else "#EF4444"
                        st.markdown(f"<strong style='color: #10B981;'>{c_val:.2f} credits</strong> &bull; <span style='font-size: 0.78rem; color: {cap_color};'>{pct_cap:.0f}% of cap</span>", unsafe_allow_html=True)
                    with c_tbtn:
                        if st.button("🗑️", key=f"dash_del_tbl_{l['id']}", help="Delete this entry and recalculate dashboard"):
