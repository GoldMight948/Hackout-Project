...
def render_circular_view():
    ...
    st.markdown("""
        <div style="margin-bottom: 20px;">
            ...
            <p style="font-size: 0.95rem; color: var(--text-muted);">
                Shift from linear "take-make-waste" disposal to closed circular economy loops: <strong>Reuse</strong>, <strong>Recycle</strong>, <strong>Recover</strong>, and <strong>Replace</strong>.
            </p>
        </div>
    ...
    tab_titles = [f"{CIRCULAR_ALTERNATIVES[k]['icon']} {CIRCULAR_ALTERNATIVES[k]['title'].split()[0]}" for k in tab_keys]
    ...
    for idx, key in enumerate(tab_keys):
        ...
        with tabs[idx]:
            st.markdown(f"""
                <div class="saas-card" style="margin-bottom: 20px;">
                    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 6px;">
                        <span style="font-size: 2.2rem;">{stream['icon']}</span>
                        <div>
            ...
            four_rs = [
                ("1. REUSE", stream["reuse"], "#059669", "🔄"),
                ("2. RECYCLE", stream["recycle"], "#10B981", "♻️"),
                ("3. RECOVER", stream["recover"], "#F59E0B", "⚡"),
                ("4. REPLACE", stream["replace"], "#6366F1", "🌱")
            ]
                        ...
                        <div class="saas-card" style="height: 100%; border-top: 4px solid {r_color}; padding: 18px; display: flex; flex-direction: column; justify-content: space-between;">
                            <div>
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                    <span style="font-size: 0.78rem; font-weight: 800; color: {r_color}; letter-spacing: 0.06em;">
                                        {r_icon} {r_label}
                                    </span>
                                </div>
                                ...
                                <div style="font-size: 0.8rem; color: #10B981; font-weight: 600; margin-bottom: 8px;">
    ...
    st.markdown("""
        <div class="saas-card">
            <div class="saas-card-title">Circular Economy Transformation Flow Example</div>
            ...
            <div style="display: flex; align-items: center; justify-content: space-around; flex-wrap: wrap; gap: 10px; text-align: center;">
                <div style="background: var(--bg-subtle); border: 1px solid var(--border-color); padding: 14px 18px; border-radius: 12px; min-width: 160px;">
                    ...
                    <div style="font-size: 0.8rem; color: #EF4444;">Landfill Liability</div>
                </div>
                <div style="font-size: 1.4rem; color: var(--text-muted);">➔</div>
                <div style="background: var(--bg-subtle); border: 1px solid var(--border-color); padding: 14px 18px; border-radius: 12px; min-width: 160px;">
                    <div style="font-size: 1.8rem;">🏭</div>
                    <strong style="font-size: 0.95rem;">Circular Supplier</strong>
                    <div style="font-size: 0.8rem; color: #3B82F6;">Certified Regrind / Recycler</div>
                </div>
                <div style="font-size: 1.4rem; color: var(--text-muted);">➔</div>
                <div style="background: var(--bg-subtle); border: 1px solid var(--border-color); padding: 14px 18px; border-radius: 12px; min-width: 160px;">
                    <div style="font-size: 1.8rem;">💵</div>
                    <strong style="font-size: 0.95rem;">Cost Recovery</strong>
                    <div style="font-size: 0.8rem; color: #059669;">Up to $8,200/yr Saved</div>
                </div>
                <div style="font-size: 1.4rem; color: var(--text-muted);">➔</div>
                <div style="background: var(--bg-subtle); border: 1px solid var(--border-color); padding: 14px 18px; border-radius: 12px; min-width: 160px;">
                    <div style="font-size: 1.8rem;">🌱</div>
                    <strong style="font-size: 0.95rem;">Carbon Abated</strong>
                    <div style="font-size: 0.8rem; color: #10B981;">Permanent Footprint Cut</div>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
