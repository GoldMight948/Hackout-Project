...
from database.db_manager import save_emissions_assessment, log_audit, get_latest_emissions
from components.icons import feather_icon, render_icon_heading, COLOR_WARNING, COLOR_SUCCESS, COLOR_NEUTRAL, COLOR_INFO

from components.csv_importer import import_past_data_from_csv, generate_blank_activity_csv_template
...
def render_upload_view():
  ...
  # Demo Datasets Quick Switcher (Strictly isolated to Demo Sessions)
  if is_demo:
    ...
    st.markdown(f"""
      <div class="saas-card" style="margin-bottom: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
          ...
          <span class="badge-medium">SANDBOX PRE-LOADED</span>
        </div>
    """, unsafe_allow_html=True)
  ...
  with c_up1:
    ...
    uploaded_file = st.file_uploader(
      ...
      key="upload_screen_file_uploader"
    )

    if uploaded_file is not None:
      last_up_file = st.session_state.get("upload_screen_last_file")
      if last_up_file != uploaded_file.name:
        with st.spinner("Processing spreadsheet and updating records..."):
          import_res = import_past_data_from_csv(uploaded_file, user_email, is_demo=is_demo)
          st.session_state["upload_screen_last_file"] = uploaded_file.name
          st.session_state.pop("data_editor_component", None)
          if import_res.get("success"):
            st.success(import_res.get("message", "File imported successfully!"))
            st.rerun()
          else:
            st.error(import_res.get("error", "Error parsing file."))
      else:
        # File has already been processed, offer quick action buttons
        col_act1, col_act2 = st.columns(2)
        with col_act1:
          if st.button("📊 View Dashboard Analysis →", type="primary", use_container_width=True, key="upload_btn_goto_dash"):
            st.session_state["nav_section"] = "dashboard"
            st.session_state["current_step"] = 5
            st.rerun()
        with col_act2:
          if st.button("📅 View Daily & Weekly Logs →", use_container_width=True, key="upload_btn_goto_logs"):
  ...
  with c_up2:
    ...
    st.markdown(f"""
      <div class="saas-card">
        <div class="saas-card-title" style="display: flex; align-items: center;">{dl_title} Template Downloads</div>
        <div class="saas-card-subtitle">
          Download pre-formatted spreadsheet templates ready for operational data entry:
        </div>
    """, unsafe_allow_html=True)
    ...
    st.download_button(
      label="📄 Download Operational Activity Logs Template (CSV)",
      data=generate_blank_activity_csv_template(),
      file_name="operational_activity_template.csv",
      mime="text/csv",
      use_container_width=True,
      key="upload_download_activity_tmpl"
    )

    st.markdown("<div style='margin-bottom: 8px;'></div>", unsafe_allow_html=True)

    st.download_button(
      label="📄 Download Facility Baseline Template (CSV)",
      data=csv_buffer,
      file_name="industrial_emissions_template.csv",
      mime="text/csv",
      use_container_width=True,
      key="upload_download_baseline_tmpl"
    )
  ...
  current_inputs = st.session_state.get("form_inputs")
  if not current_inputs:
    db_emissions = get_latest_emissions(user_email)
    if db_emissions:
      current_inputs = dict(db_emissions)
    elif is_demo:
      current_inputs = DEMO_BUSINESSES["manufacturing_plant"]["data"].copy()
    else:
      current_inputs = {}

  grid_rows = [
    {"Category": "Energy", "Metric": "Electricity Consumption", "Value": safe_float(current_inputs.get("electricity_kwh"), 0.0 if not is_demo else 350000.0), "Unit": "kWh / yr"},
    {"Category": "Energy", "Metric": "On-Site Renewable Energy", "Value": safe_float(current_inputs.get("renewable_pct"), 0.0 if not is_demo else 15.0), "Unit": "% Share"},
    {"Category": "Energy", "Metric": "Diesel Consumption", "Value": safe_float(current_inputs.get("diesel_liters"), 0.0 if not is_demo else 12000.0), "Unit": "Liters / yr"},
    {"Category": "Energy", "Metric": "Petrol / Gasoline", "Value": safe_float(current_inputs.get("petrol_liters"), 0.0 if not is_demo else 4500.0), "Unit": "Liters / yr"},
    {"Category": "Energy", "Metric": "Natural Gas Consumption", "Value": safe_float(current_inputs.get("gas_m3"), 0.0 if not is_demo else 32000.0), "Unit": "m³ / yr"},
    {"Category": "Transport", "Metric": "Truck Freight Distance", "Value": safe_float(current_inputs.get("truck_km"), 0.0 if not is_demo else 55000.0), "Unit": "km / yr"},
    {"Category": "Transport", "Metric": "Company Car Distance", "Value": safe_float(current_inputs.get("car_km"), 0.0 if not is_demo else 18000.0), "Unit": "km / yr"},
    {"Category": "Transport", "Metric": "Employee Commute", "Value": safe_float(current_inputs.get("commute_km"), 0.0 if not is_demo else 75000.0), "Unit": "km / yr"},
    {"Category": "Transport", "Metric": "Delivery Fleet Vehicles", "Value": safe_float(current_inputs.get("delivery_vehicles"), 0.0 if not is_demo else 6), "Unit": "Count"},
    {"Category": "Waste", "Metric": "Organic Waste", "Value": safe_float(current_inputs.get("organic_waste_kg"), 0.0 if not is_demo else 24000.0), "Unit": "kg / yr"},
    {"Category": "Waste", "Metric": "Plastic Waste", "Value": safe_float(current_inputs.get("plastic_waste_kg"), 0.0 if not is_demo else 16000.0), "Unit": "kg / yr"},
    {"Category": "Waste", "Metric": "Metal Waste", "Value": safe_float(current_inputs.get("metal_waste_kg"), 0.0 if not is_demo else 8500.0), "Unit": "kg / yr"},
    {"Category": "Waste", "Metric": "Paper & Cardboard", "Value": safe_float(current_inputs.get("paper_waste_kg"), 0.0 if not is_demo else 14000.0), "Unit": "kg / yr"},
    {"Category": "Waste", "Metric": "Hazardous Waste", "Value": safe_float(current_inputs.get("hazardous_waste_kg"), 0.0 if not is_demo else 1200.0), "Unit": "kg / yr"},
    {"Category": "Water", "Metric": "Water Consumption", "Value": safe_float(current_inputs.get("water_m3"), 0.0 if not is_demo else 6400.0), "Unit": "m³ / yr"},
    {"Category": "Water", "Metric": "Wastewater Generated", "Value": safe_float(current_inputs.get("wastewater_m3"), 0.0 if not is_demo else 5200.0), "Unit": "m³ / yr"},
    {"Category": "Manufacturing", "Metric": "Raw Material Used", "Value": safe_float(current_inputs.get("raw_material_tonnes"), 0.0 if not is_demo else 380.0), "Unit": "tonnes / yr"},
    {"Category": "Manufacturing", "Metric": "Production Output Units", "Value": safe_float(current_inputs.get("production_units"), 0.0 if not is_demo else 150000.0), "Unit": "units / yr"},
    {"Category": "Manufacturing", "Metric": "Machine Running Hours", "Value": safe_float(current_inputs.get("machine_hours"), 0.0 if not is_demo else 3200.0), "Unit": "hrs / yr"},
    {"Category": "Carbon Credits", "Metric": "Allocated Carbon Credits", "Value": safe_float(current_inputs.get("total_credits"), 0.0 if not is_demo else 350.0), "Unit": "Credits (t CO2)"},
    {"Category": "Carbon Credits", "Metric": "Carbon Credit Price", "Value": safe_float(current_inputs.get("credit_price"), 0.0 if not is_demo else 38.0), "Unit": "₹ / Credit"}
  ]
