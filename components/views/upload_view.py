...
=======
        {"Category": "Energy", "Metric": "Electricity Consumption", "Value": float(current_inputs.get("electricity_kwh", 350000.0 if is_demo else 0.0)), "Unit": "kWh / yr"},
        {"Category": "Energy", "Metric": "On-Site Renewable Energy", "Value": float(current_inputs.get("renewable_pct", 15.0 if is_demo else 0.0)), "Unit": "% Share"},
        {"Category": "Energy", "Metric": "Diesel Consumption", "Value": float(current_inputs.get("diesel_liters", 12000.0 if is_demo else 0.0)), "Unit": "Liters / yr"},
        {"Category": "Energy", "Metric": "Petrol / Gasoline", "Value": float(current_inputs.get("petrol_liters", 4500.0 if is_demo else 0.0)), "Unit": "Liters / yr"},
        {"Category": "Energy", "Metric": "Natural Gas Consumption", "Value": float(current_inputs.get("gas_m3", 32000.0 if is_demo else 0.0)), "Unit": "m³ / yr"},
        {"Category": "Transport", "Metric": "Truck Freight Distance", "Value": float(current_inputs.get("truck_km", 55000.0 if is_demo else 0.0)), "Unit": "km / yr"},
        {"Category": "Transport", "Metric": "Company Car Distance", "Value": float(current_inputs.get("car_km", 18000.0 if is_demo else 0.0)), "Unit": "km / yr"},
        {"Category": "Transport", "Metric": "Employee Commute", "Value": float(current_inputs.get("commute_km", 75000.0 if is_demo else 0.0)), "Unit": "km / yr"},
        {"Category": "Transport", "Metric": "Delivery Fleet Vehicles", "Value": float(current_inputs.get("delivery_vehicles", 6 if is_demo else 0)), "Unit": "Count"},
        {"Category": "Waste", "Metric": "Organic Waste", "Value": float(current_inputs.get("organic_waste_kg", 24000.0 if is_demo else 0.0)), "Unit": "kg / yr"},
        {"Category": "Waste", "Metric": "Plastic Waste", "Value": float(current_inputs.get("plastic_waste_kg", 16000.0 if is_demo else 0.0)), "Unit": "kg / yr"},
        {"Category": "Waste", "Metric": "Metal Waste", "Value": float(current_inputs.get("metal_waste_kg", 8500.0 if is_demo else 0.0)), "Unit": "kg / yr"},
        {"Category": "Waste", "Metric": "Paper & Cardboard", "Value": float(current_inputs.get("paper_waste_kg", 14000.0 if is_demo else 0.0)), "Unit": "kg / yr"},
        {"Category": "Waste", "Metric": "Hazardous Waste", "Value": float(current_inputs.get("hazardous_waste_kg", 1200.0 if is_demo else 0.0)), "Unit": "kg / yr"},
        {"Category": "Water", "Metric": "Water Consumption", "Value": float(current_inputs.get("water_m3", 6400.0 if is_demo else 0.0)), "Unit": "m³ / yr"},
        {"Category": "Water", "Metric": "Wastewater Generated", "Value": float(current_inputs.get("wastewater_m3", 5200.0 if is_demo else 0.0)), "Unit": "m³ / yr"},
        {"Category": "Manufacturing", "Metric": "Raw Material Used", "Value": float(current_inputs.get("raw_material_tonnes", 380.0 if is_demo else 0.0)), "Unit": "tonnes / yr"},
        {"Category": "Manufacturing", "Metric": "Production Output Units", "Value": float(current_inputs.get("production_units", 150000.0 if is_demo else 0.0)), "Unit": "units / yr"},
        {"Category": "Manufacturing", "Metric": "Machine Running Hours", "Value": float(current_inputs.get("machine_hours", 3200.0 if is_demo else 0.0)), "Unit": "hrs / yr"},
...
=======
            new_inputs["electricity_kwh"] = float(val_map.get("Electricity Consumption", 0.0))
            new_inputs["renewable_pct"] = float(val_map.get("On-Site Renewable Energy", 0.0))
            new_inputs["diesel_liters"] = float(val_map.get("Diesel Consumption", 0.0))
            new_inputs["petrol_liters"] = float(val_map.get("Petrol / Gasoline", 0.0))
            new_inputs["gas_m3"] = float(val_map.get("Natural Gas Consumption", 0.0))
            new_inputs["truck_km"] = float(val_map.get("Truck Freight Distance", 0.0))
            new_inputs["car_km"] = float(val_map.get("Company Car Distance", 0.0))
            new_inputs["commute_km"] = float(val_map.get("Employee Commute", 0.0))
            new_inputs["delivery_vehicles"] = int(val_map.get("Delivery Fleet Vehicles", 0))
            new_inputs["organic_waste_kg"] = float(val_map.get("Organic Waste", 0.0))
            new_inputs["plastic_waste_kg"] = float(val_map.get("Plastic Waste", 0.0))
            new_inputs["metal_waste_kg"] = float(val_map.get("Metal Waste", 0.0))
            new_inputs["paper_waste_kg"] = float(val_map.get("Paper & Cardboard", 0.0))
            new_inputs["hazardous_waste_kg"] = float(val_map.get("Hazardous Waste", 0.0))
            new_inputs["water_m3"] = float(val_map.get("Water Consumption", 0.0))
            new_inputs["wastewater_m3"] = float(val_map.get("Wastewater Generated", 0.0))
            new_inputs["raw_material_tonnes"] = float(val_map.get("Raw Material Used", 0.0))
            new_inputs["production_units"] = float(val_map.get("Production Output Units", 0.0))
            new_inputs["machine_hours"] = float(val_map.get("Machine Running Hours", 0.0))
            new_inputs["total_credits"] = float(val_map.get("Allocated Carbon Credits", def_credits))
            new_inputs["credit_price"] = float(val_map.get("Carbon Credit Price", def_price))
>>>>>>> Stashed changes
