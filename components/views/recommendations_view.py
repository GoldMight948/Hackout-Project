...
def render_recommendations_view():
  ...
  st.markdown(f"""
    <div style="background: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.3); border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
      <div style="display: flex; align-items: center;">
        {feather_icon('target', color='#CBDED3', size=22, margin_right=10)}
        <div>
  ...
  for idx, rec in enumerate(filtered_recs):
    ...
    card_icon = feather_icon(rec_f_icon, color="#CBDED3", size=16, margin_right=5)
    ...
    with st.container():
      st.markdown(f"""
        <div class="saas-card" style="margin-bottom: 14px; border-left: 5px solid {'#CBDED3' if rec.get('targeted_hotspot_rank', 99) == 1 else ('#F59E0B' if rec.get('targeted_hotspot_rank', 99) == 2 else ('#3B82F6' if rec.get('targeted_hotspot_rank', 99) == 3 else '#64748B'))};">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 8px;">
            <div>
              <div style="display: flex; align-items: center; flex-wrap: wrap; gap: 6px;">
                {hotspot_badge}
                <span style="font-size: 0.78rem; font-weight: 700; color: #CBDED3; text-transform: uppercase; display: flex; align-items: center;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 12px; padding: 12px; background: var(--bg-subtle); border-radius: 10px; border: 1px solid var(--border-color); font-size: 0.85rem;">
            <div>
              ...
              <strong style="color: #CBDED3; font-size: 0.95rem;">-{rec['co2_saved_t']} t/yr ({rec['co2_saved_pct']}%)</strong>
            </div>
            <div>
              ...
              <strong style="color: #B0C5BA; font-size: 0.95rem;">+₹{rec.get('annual_savings_inr', 0):,.0f}/yr</strong>
            </div>
            <div>
            ...
            <div>
              ...
              <strong style="color: #B0C5BA;">{rec['payback_time']}</strong>
            </div>
            <div>
      ...
      with st.expander(f"📖 Detailed Engineering, Circular Benefits & Government Grants"):
        ...
        with c_exp1:
          st.markdown(f"**Circular Economy Benefit:**")
          st.write(rec["circular_benefit"])
        with c_exp2:
          st.markdown(f"**Government Incentives & Subsidies:**")
