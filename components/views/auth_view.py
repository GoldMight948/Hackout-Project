...
def render_auth_view():
  ...
  auth_mode = st.session_state.get("auth_tab", "login")
  if auth_mode == "register":
    tab_register, tab_demo = st.tabs(["Create Business Profile", "1-Click Demo Profiles"])
  else:
    tab_login, tab_demo = st.tabs(["Sign In", "1-Click Demo Profiles"])
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
            <div>
              <div style="font-weight: 700; font-size: 0.95rem; color: #065F46; display: flex; align-items: center;">
                {feather_icon('award', color='#5A8766', size=16, margin_right=6)} Statutory Carbon Credit Quota (EPA / CARB / CCTS Calibrated)
              </div>
            ...
            <div style="text-align: right;">
              <div style="font-size: 1.65rem; font-weight: 800; color: #5A8766; line-height: 1.1;">
        ...
        st.markdown(f"""
          <div style="background: var(--bg-subtle); border: 1px solid var(--border-color); border-radius: 12px; padding: 14px; height: 170px; display: flex; flex-direction: column; justify-content: space-between;">
            <div>
              ...
              <div style="font-size: 0.74rem; color: #5A8766; font-weight: 600; margin-top: 4px;">{preset['type_label']}</div>
            </div>
          </div>
        """, unsafe_allow_html=True)
